...
### 진행(2026-08-26) — §6 #4 성과 측정
- 실판매(income/list.asp): 6/7 이후 첫 판매가 8월에 재개, **2건**. (1) 실험동물기술원 2급 필기 기출복기 pk=30012119, 수익합계 4,800원, 유입 m.blog.naver.com (2) 세브란스 CPR PPT pk=20071734, 수익합계 2,000원, 유입 미상(Null). 7월 0건. 둘 다 레포트 → down.asp 최근 1개월 레포트 2건과 일치.
- 유입경로(refer.asp, 8/19~8/26, 465접속): 네이버 계열 약 58%(m.blog.naver 43%+blog.naver 12%+naver 등), Null 42%(194), kakao.ivyro 1건, dothome·cafe24·blogger 0.
- 해석: 유입·판매 모두 네이버 주도. README §3 기준선(8/16 네이버 19%·Null 80%)보다 네이버 비중 19%→58% 급증. 내가 게시 가능한 4개 블로그는 유입·판매 사실상 0 지속.
- 시사점: 레버는 여전히 네이버 → posts_naver/네이버_변형본_6건.md 하루 1개씩 게시가 최우선. 8월 판매 2건이 모두 레포트(개강 최상위)라 9월 레포트 우선 배치 유효.
### 진행(2026-08-26 #2) — §6 #6 hadesyi 카테고리 정리 + 내부링크
- 실제 상태: 카테고리 체계는 이미 존재(레포트·실습일지·시험자료·면접자료·서평감상·전문자료·자기소개). 우리 글(leesk55 포함) 100건 중 대부분 이미 분류돼 있었고, **미분류는 최근 플래그십 7건뿐**이었음. (미분류 전체 4,962건은 자동 포스터가 만든 잡글로 우리 것 아님 → 손대지 않음.)
- 7건 분류 완료: 14222·14224·14227(레포트) / 14229(자기소개) / 14232·14234(시험자료) / 14233(실습일지). WordPress REST API(wp.apiFetch) 사용.
- 내부링크 추가: 7건 각각에 "함께 보면 좋은 글" 섹션으로 관련 글 2개씩 상호 링크(같은 주제군끼리). SEO 내부링크 개선.
- 참고: hadesyi는 총 5,633글(대부분 자동 포스터). 카테고리 대량 정리는 범위·리스크 커서 보류. 우리 제휴글만 관리 권장.
